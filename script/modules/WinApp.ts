...
class WinApp {
  ...
  static startApp() {
    ...
    app.whenReady().then(async () => {
      // await this.checkAndInstallVirtualAudioCapturer()
      this.ipcListening()
      WinMain.create()
  ...
  static async checkAndInstallVirtualAudioCapturer(): Promise<any> {
    const installScriptPath = path.join(__dirname, "install-virtual-audio-capturer.bat")
    const installedMarkerPath = path.join(app.getPath("userData"), "virtual-audio-capturer-installed")

    // 检查是否已经安装
    if (!fs.existsSync(installedMarkerPath)) {
      // 执行安装脚本
      await new Promise((resolve: any, reject) => {
        execFile(installScriptPath, (error, stdout, stderr) => {
          if (error) {
            console.error(`Error installing virtual-audio-capturer: ${error.message}`)
            reject(error)
          } else {
            console.log(`Installation completed: ${stdout}`)
            fs.writeFileSync(installedMarkerPath, "")
            return resolve()
          }
        })
      })
    } else {
      console.log("virtual-audio-capturer is already installed.")
    }
  }
}
